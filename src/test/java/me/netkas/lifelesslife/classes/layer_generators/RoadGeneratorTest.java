...
class RoadGeneratorTest {
    ...
    void generateLowDensity() {
        ...
        (new MainStreetGenerator()).generateLayer(chunk, DensityLevel.LOW, new Random());
        (new RoadGenerator()).generateLayer(chunk, DensityLevel.LOW, new Random());

        try
        {
            renderChunk(chunk, "low_density_streets.png");
        }
        catch(IOException e)
        {
            Assertions.fail(e);
    ...
    void generateMediumDensity() {
        AreaChunk chunk = new AreaChunk(512,  512);
        (new MainStreetGenerator()).generateLayer(chunk, DensityLevel.MEDIUM, new Random());
        (new RoadGenerator()).generateLayer(chunk, DensityLevel.MEDIUM, new Random());

        try
        {
            renderChunk(chunk, "medium_density_streets.png");
        }
        catch(IOException e)
        {
            Assertions.fail(e);
    ...
    void generateHighDensity() {
        AreaChunk chunk = new AreaChunk(512,  512);
        (new MainStreetGenerator()).generateLayer(chunk, DensityLevel.HIGH, new Random());
        (new RoadGenerator()).generateLayer(chunk, DensityLevel.HIGH, new Random());

        try
        {
            renderChunk(chunk, "high_density_streets.png");
        }
        catch(IOException e)
        {
            Assertions.fail(e);
    ...
    private void renderChunk(AreaChunk chunk, String name) throws IOException {
        ...
        for(AreaRegion region : chunk.getRegions(AreaRegionType.ROAD))
        {
            for(Point point : region.getRegion().getPoints())
            {
                graphics.fillRect(point.x() * tileSize, point.y() * tileSize, tileSize, tileSize);
